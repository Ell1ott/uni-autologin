...
if (isBroker()) {
  chrome.storage.sync.get(
    ["ual_active", "ual_auto_choose_login", "ual_username", "ual_password"],
    (data) => {
      if (data.ual_active) {
        if (f1.ifElementExists("#username")) {
          chrome.storage.local.get(
            ["ual_last_timestamp_username"],
            (timedata) => {
              ...
              if (!delay_ok) {
                ...
                alert(
                  "Grundet en fejl ved login er autologin blevet slået fra. Tjek om dit brugernavn er korrekt."
                );
              }
